...
export const ManipulationChainVisualizer: React.FC<ManipulationChainVisualizerProps> = ({ steps }) => {
  ...
  return (
    <div id="cybersentinel-manipulation-chain" className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
      
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-3 mb-5 border-b border-slate-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3 relative">
            {steps.map((step, idx) => (
              <div
                ...
                className="relative bg-slate-950/80 border border-slate-800 rounded-xl p-3.5 flex flex-col justify-between hover:border-purple-500/40 transition-all group"
              >
                {/* Step Index Badge */}
                ...
                <div className="space-y-2 pt-2 border-t border-slate-800/80 text-[11px]">
                  <div>
                    <span className="text-slate-500 uppercase tracking-wider text-[9px] block font-semibold">Attacker Hook</span>
                    <p className="text-slate-300 leading-tight">{step.psychologicalMechanism}</p>
                  </div>
                  <div>
                    <span className="text-slate-500 uppercase tracking-wider text-[9px] block font-semibold">Target Victim Reaction</span>
                    <p className="text-amber-300/90 leading-tight italic">"{step.victimReaction}"</p>
                  </div>
                </div>
              </div>
            ))}
