...
export const ProjectedImpactSimulator: React.FC<ProjectedImpactSimulatorProps> = ({ impact }) => {
  ...
  return (
    <div id="cybersentinel-projected-impact-panel" className="bg-slate-900/90 border border-slate-800 rounded-2xl p-5 sm:p-6 shadow-xl">
      ...
      {impact.timeline.length > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {impact.timeline.map((step) => (
              <div
                ...
                className="p-3.5 bg-slate-950/80 rounded-xl border border-slate-800 flex flex-col justify-between space-y-2 hover:border-slate-700 transition-all"
              >
                <div>
                  ...
                  <div className="text-[11px] text-slate-400 leading-tight mb-2">
                    <span className="text-slate-500 font-semibold block text-[9px] uppercase">Technical Execution:</span>
                    {step.systemResponse}
                  </div>
                </div>
        ...
        <div className="p-4 bg-slate-950/40 rounded-xl text-center text-xs text-emerald-400 border border-emerald-500/20">
