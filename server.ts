...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    // If no API key, use dynamic heuristic analyzer
    if (!process.env.GEMINI_API_KEY) {
      const heuristic = generateDynamicHeuristicAnalysis(input);
      analysisCache.set(cacheKey, heuristic);
      return res.json(heuristic);
    }

    const systemPrompt = `You are CyberSentinel, a world-class AI Cybersecurity Threat Intelligence & Social Engineering Detection Engine.
You analyze incoming emails, messages, SMS, DMs, URLs, and text to detect phishing, spear-phishing, BEC, smishing, credential theft, and psychological manipulation.

Perform a rigorous 10-stage threat analysis and return a structured JSON response matching the schema.`;

    const userPrompt = `Analyze the following suspicious communication:
Type: ${input.type}
Claimed Organization: ${input.claimedOrganization || 'Not specified'}
Sender Name: ${input.senderName || 'Not specified'}
Sender Address/Phone: ${input.senderEmailOrPhone || 'Not specified'}
...
"""`;
    ...
    try {
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              humanManipulationRiskScore: { type: Type.INTEGER },
              technicalThreatRiskScore: { type: Type.INTEGER },
              ...
              nlpLayer: {
                ...
                properties: {
                  ...
                  coerciveLanguageScore: { type: Type.INTEGER },
                  grammaticalAnomalies: { type: Type.ARRAY, items: { type: Type.STRING } },
                  homoglyphOrEvasiveTricks: { type: Type.ARRAY, items: { type: Type.STRING } },
                  linguisticMarkers: { type: Type.ARRAY, items: { type: Type.STRING } },
                },
                required: ['tone', 'sentiment', 'coerciveLanguageScore', 'grammaticalAnomalies', 'homoglyphOrEvasiveTricks', 'linguisticMarkers']
              },
              socialEngineeringLayer: {
                type: Type.OBJECT,
                properties: {
                  primaryTactic: { type: Type.STRING },
                  ...
                  emotionalManipulationScore: { type: Type.INTEGER }
                },
                required: ['primaryTactic', 'psychologicalTriggers', 'emotionalManipulationScore']
              },
              urlLayer: {
                type: Type.OBJECT,
                properties: {
                  detectedUrls: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        domainAgeEstimate: { type: Type.STRING },
                        typosquattingRisk: { type: Type.STRING },
                        lookalikeTarget: { type: Type.STRING },
                        tldRisk: { type: Type.STRING },
                        destinationMismatch: { type: Type.BOOLEAN },
                        ipBasedUrl: { type: Type.BOOLEAN },
                        findings: { type: Type.ARRAY, items: { type: Type.STRING } }
                      },
                      required: ['originalUrl', 'isSuspicious', 'domain', 'typosquattingRisk', 'tldRisk', 'destinationMismatch', 'ipBasedUrl', 'findings']
                    }
                  },
                  urlRiskScore: { type: Type.INTEGER }
                },
                required: ['detectedUrls', 'urlRiskScore']
              },
              senderIdentityLayer: {
                type: Type.OBJECT,
                properties: {
                  claimedIdentity: { type: Type.STRING },
                  actualSender: { type: Type.STRING },
                  isSpoofed: { type: Type.BOOLEAN },
                  freeWebmailDiscrepancy: { type: Type.BOOLEAN },
                  replyToMismatch: { type: Type.BOOLEAN },
                  domainAlignment: { type: Type.STRING },
                  authHealth: {
                    type: Type.OBJECT,
                    properties: {
                  ...
                  identityRiskScore: { type: Type.INTEGER }
                },
                required: ['claimedIdentity', 'actualSender', 'isSpoofed', 'freeWebmailDiscrepancy', 'replyToMismatch', 'domainAlignment', 'authHealth', 'identityRiskScore']
              },
              behavioralLayer: {
                type: Type.OBJECT,
                properties: {
                  urgencyWindow: { type: Type.STRING },
                  protocolBypassAttempt: { type: Type.BOOLEAN },
                  outOfBandCommunicationRequested: { type: Type.BOOLEAN },
                  sensitiveActionRequested: { type: Type.STRING },
                  behaviorRiskScore: { type: Type.INTEGER }
                },
                required: ['urgencyWindow', 'protocolBypassAttempt', 'outOfBandCommunicationRequested', 'sensitiveActionRequested', 'behaviorRiskScore']
              },
              contextConsistencyLayer: {
                type: Type.OBJECT,
                properties: {
                  organizationContextMismatch: { type: Type.STRING },
                  channelAppropriateness: { type: Type.STRING },
                  historicalBaselineDeviation: { type: Type.STRING },
                  consistencyScore: { type: Type.INTEGER }
                },
                required: ['organizationContextMismatch', 'channelAppropriateness', 'historicalBaselineDeviation', 'consistencyScore']
              },
              attackDNA: {
                type: Type.ARRAY,
                items: {
                  ...
                  properties: {
                    ...
                    mitreRef: { type: Type.STRING },
                    description: { type: Type.STRING },
                    evidence: { type: Type.STRING }
                  },
                  required: ['id', 'name', 'severity', 'description', 'evidence']
                }
              },
              manipulationChain: {
                type: Type.ARRAY,
                items: {
                  ...
                  properties: {
                    ...
                    victimReaction: { type: Type.STRING },
                    iconName: { type: Type.STRING }
                  },
                  required: ['stepNumber', 'phase', 'trigger', 'psychologicalMechanism', 'victimReaction', 'iconName']
                }
              },
              trustGraph: {
                type: Type.OBJECT,
                  ...
                  nodes: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        isCompromisedOrMalicious: { type: Type.BOOLEAN },
                        statusText: { type: Type.STRING }
                      },
                      required: ['id', 'label', 'type', 'isCompromisedOrMalicious', 'statusText']
                    }
                  },
                  edges: {
                    type: Type.ARRAY,
                    items: {
                      ...
                      properties: {
                        ...
                        to: { type: Type.STRING },
                        label: { type: Type.STRING },
                        isBrokenTrust: { type: Type.BOOLEAN },
                        reason: { type: Type.STRING }
                      },
                      required: ['from', 'to', 'label', 'isBrokenTrust', 'reason']
                    }
                  },
              ...
              projectedImpact: {
                ...
                properties: {
                  ...
                  financialExposureEstimate: { type: Type.STRING },
                  dataCompromiseTypes: { type: Type.ARRAY, items: { type: Type.STRING } },
                  ...
                  timeline: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        attackerOutcome: { type: Type.STRING },
                        riskLevel: { type: Type.STRING }
                      },
                      required: ['stage', 'action', 'systemResponse', 'attackerOutcome', 'riskLevel']
                    }
                  }
                },
                required: ['severityLevel', 'financialExposureEstimate', 'dataCompromiseTypes', 'blastRadius', 'timeline']
              },
              highlightedFlags: {
                type: Type.ARRAY,
                items: {
                  ...
                  required: ['text', 'category', 'explanation', 'severity']
                }
              },
              whyDangerousExplanation: { type: Type.ARRAY, items: { type: Type.STRING } },
              recommendations: {
                type: Type.ARRAY,
                items: {
                  type: Type.OBJECT,
                  ...
                  required: ['priority', 'action', 'detail', 'iconName']
                }
              },
              mitreAttackMappings: { type: Type.ARRAY, items: { type: Type.STRING } }
            },
            required: [
              'scanId', 'timestamp', 'overallRiskScore', 'humanManipulationRiskScore',
              'technicalThreatRiskScore', 'threatLevel', 'threatCategory', 'categoryName',
              'confidenceScore', 'executiveVerdict', 'nlpLayer', 'socialEngineeringLayer',
              'urlLayer', 'senderIdentityLayer', 'behavioralLayer', 'contextConsistencyLayer',
              'attackDNA', 'manipulationChain', 'trustGraph', 'projectedImpact',
              'highlightedFlags', 'whyDangerousExplanation', 'recommendations', 'mitreAttackMappings'
            ]
          }
        }
      });
      ...
      if (!parsedJson.scanId) parsedJson.scanId = `CS-${Date.now().toString(36).toUpperCase()}`;
      if (!parsedJson.timestamp) parsedJson.timestamp = new Date().toISOString();

      analysisCache.set(cacheKey, parsedJson);
      return res.json(parsedJson);
    } catch (geminiError: any) {
      console.warn('Notice: Gemini API responded with rate limit / quota or transient condition. Using Dynamic Threat Intelligence Engine.');
      const fallback = generateDynamicHeuristicAnalysis(input);
