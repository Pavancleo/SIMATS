...
export const TrustGraphVisualizer: React.FC<TrustGraphVisualizerProps> = ({ trustGraph }) => {
  ...
  return (
    <div className="container">
      <div id="cybersentinel-trust-graph-panel" className="card p-5 sm:p-6 shadow-xl relative overflow-hidden">
        ...
        {/* Summary Rationale */}
        <div className="p-3 bg-slate-950/70 rounded-xl border border-slate-800 mb-4 text-xs text-slate-300 flex items-start gap-2 relative z-10">
          ...
          {trustGraph.nodes.map((node, idx) => (
            <div key={node.id} className="container">
              <div
                ...
                className={`card p-3.5 flex flex-col justify-between transition-all ${
                  node.isCompromisedOrMalicious
                    ? 'border-rose-500/50 text-rose-200 shadow-sm shadow-rose-950/30'
                    : 'text-slate-200'
                }`}
              >
                <div>
                  ...
                  <div className="innerText text-xs font-bold truncate mb-1" title={node.label}>
                    {node.label}
                  </div>
                </div>
