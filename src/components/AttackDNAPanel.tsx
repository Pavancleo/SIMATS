...
export const AttackDNAPanel: React.FC<AttackDNAPanelProps> = ({ techniques }) => {
  ...
  return (
    <div className="container">
      <div id="cybersentinel-attack-dna-panel" className="card p-5 sm:p-6 shadow-xl relative overflow-hidden">
        
        {/* Header */}
        <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-800 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5 relative z-10">
            {techniques.map((tech) => (
              <div key={tech.id} className="container">
                <div
                  id={`dna-card-${tech.id}`}
                  className="card p-4 flex flex-col justify-between hover:border-slate-500 transition-all group"
                >
                  <div>
                    ...
                    <h4 className="innerText text-xs font-bold transition-colors mb-1.5">
                      {tech.name}
                    </h4>

                    <p className="desc text-xs text-slate-400 leading-relaxed mb-3">
                      {tech.description}
                    </p>
                  </div>
